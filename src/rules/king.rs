...
pub fn is_valid_move(start_rank: u8, start_file: u8, target_rank: u8, target_file: u8, chess_board: &ChessBoard) -> bool {
    ...
    log("NOTE", "crate::rules::king::is_valid_move does not currently check for and allow castling.");
    ...
    if start_rank >= 8 || start_file >= 8 {
        log(
            "ERROR",
            format!("crate::rules::king::is_valid_move received an invalid starting position: file #{}, rank #{}", start_file, start_rank)
        );
    ...
    || ( (start_rank as i8 - target_rank as i8).abs() > 1 ) {
        log("INFO", "Bad move: The target position is not within one file and one rank of the starting position.");
        return false;
    }
    log("INFO", "The target position is within one file and one rank of the starting position.");
